...
class Navigator {
	...
	public constructor(wrapperObj: any) {
		this.init(wrapperObj);
	}
	...
	public drawOutline(
		...
		if (inverted) {
			...
			path = [
				...
				["L", left + height, navigatorTop + navigatorSize + scrollButtonSize],
			];
			if (maskInside) {
				path.push(
					...
					// upper right of z.r.
					["L", left + height, zoomedMax + halfOutline],
				);
			}
		} else {
	...
	public drawMasks(
		...
		navigator.shades.forEach((shade: SVGElement, i: number): void => {
			shade[verb]({
				x: x[i],
				y: y[i],
				width: width[i],
	...
	public renderElements(): void {
		...
		[!maskInside, maskInside, !maskInside].forEach(
			(hasMask: boolean | undefined, index: number): void => {
				const shade = renderer
					.rect()
					.addClass(
	...
	public render(
		...
		let navigatorWidth, navigatorSize, verb;
		...
		let zoomedMax =
			(navigator.size =
			navigatorSize =
				pick(xAxis.len, inverted ? chart.plotHeight : chart.plotWidth));

		if (inverted) {
			navigatorWidth = 0;
		} else {
			navigatorWidth = navigatorSize;
		}
		...
		if (currentRange < (minRange as any)) {
			if (this.grabbedLeft) {
				pxMin = xAxis.toPixels(newMax - (minRange as any) - pointRange, true);
			} else if (this.grabbedRight) {
				pxMax = xAxis.toPixels(newMin + (minRange as any) + pointRange, true);
			}
		} else if (
			defined(maxRange) &&
			...
			if (this.grabbedLeft) {
				pxMin = xAxis.toPixels(newMax - (maxRange as any) - pointRange, true);
			} else if (this.grabbedRight) {
				pxMax = xAxis.toPixels(newMin + (maxRange as any) + pointRange, true);
			}
		}

		// Handles are allowed to cross, but never exceed the plot area
		navigator.zoomedMax = clamp(Math.max(pxMin, pxMax as any), 0, zoomedMax);
	...
	public addMouseEvents(): void {
		...
		if (navigator.series && navigator.series[0]) {
			eventsToUnbind.push(
				addEvent(navigator.series[0].xAxis, "foundExtremes", function (): void {
	 ...
	 */
	public shadesMousedown(e: PointerEvent, index: number): void {
	...
	public onMouseMove(e: PointerEvent): void {
		...
		if (!(e as any).touches || (e as any).touches[0].pageX !== 0) {
			...
			if (navigator.grabbedLeft) {
				...
				navigator.hasDragged = true;
				if (chartX < (dragOffset as any)) {
					// outside left
					chartX = dragOffset;
					// outside right
				} else if (chartX > navigatorSize + (dragOffset as any) - range) {
	...
	public init(chart: Chart): void {
		const chartOptions = chart.options,
			...
			isInverted = chart.options.chart.inverted;

		this.options = chart.options.navigator;
		...
		this.opposite = pick(
			...
			Boolean(!navigatorEnabled && isInverted),
		); // #6262
		...
		chart.isDirtyBox = true;
			...
			navigator.reversedExtremes =
				(isInverted && !navigator.xAxis.reversed) ||
				(!isInverted && navigator.xAxis.reversed);
	...
	public setBaseSeries(
		...
		baseSeriesOptions =
			baseSeriesOptions ||
			this.options?.baseSeries ||
			(chart.series.length
				? // Find the first non-navigator series (#8430)
				  (
						find(
							chart.series,
							(s: Series): boolean => !s.options.isInternal,
						) as any
				  ).index
				: 0);
	 ...
	 */
	public updateNavigatorSeries(addEvents: boolean, redraw?: boolean): void {
		const navigator = this,
			chart = navigator.chart,
			baseSeries = navigator.baseSeries,
			navSeriesMixin = {
			...
			navigatorSeries = (navigator.series = (navigator.series || []).filter(
				(navSeries): boolean => {
					const base = navSeries.baseSeries;

					if (baseSeries.indexOf(base as any) < 0) {
						// Not in array
		...
		if (
			...
			chartNavigatorSeriesOptions = splat(chartNavigatorSeriesOptions) as any;
			(chartNavigatorSeriesOptions as any).forEach(
				(userSeriesOptions: SeriesTypeOptions, i: number): void => {
	...
	public addBaseSeriesEvents(): void {
		...
		if (baseSeries[0] && baseSeries[0].xAxis) {
			baseSeries[0].eventsToUnbind.push(
				addEvent(
					baseSeries[0].xAxis,
					"foundExtremes",
					this.modifyBaseAxisExtremes,
	...
	public getBaseSeriesMin(currentSeriesMin: number): number {
		return this.baseSeries.reduce(function (
			min: number,
			series: Series,
		): number {
			// (#10193)
			return Math.min(
				min,
				series.xData && series.xData.length ? series.xData[0] : min,
			);
		}, currentSeriesMin);
	}
	...
	public modifyBaseAxisExtremes(this: Axis): void {
		...
		// Reset
		(navigator as any).stickToMin = (navigator as any).stickToMax = null as any;
	}
	...
	public destroy(): void {
		...
		[this.handles].forEach((coll: Array<SVGElement>): void => {
